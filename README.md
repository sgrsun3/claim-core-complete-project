...
### 2026-08-06 重构轻量岗位任务 Demo 控制器

- **按岗位切换**：按查勘、定损、核赔三个任务环节，分别展示作业岗与管理岗，共 6 个岗位入口。
- **双流程切换**：新增「作业岗流程 / 管理岗流程」切换，同一视图只展示同类岗位，避免作业岗与管理岗交错；切换后保留当前查勘、定损或核赔环节。
- **岗位职责可见**：切换岗位时同步展示当前岗位职责，岗位入口补充该岗位的核心工作说明。
- **任务步骤可操作**：每个岗位仅展示自身任务步骤，步骤之间增加方向箭头；保留原有首页待办、任务处理、Skill 反馈、修改、调试和发布触发逻辑。
- **横向轻量浮层**：控制器由大面积面板压缩为约 140 px 高的横向透明浮层，并进一步降低面板、卡片、边框与阴影的不透明度，减少对首页和任务工作区的遮挡。
- 涉及文件：`index.html`。
